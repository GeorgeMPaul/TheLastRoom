...
- `level-02.js` is real-authored against `level-02.glb` (Saturday-night hangout). 14 interactives wired. L2-specific meshes in the current GLB: `Culprit` (visitor silhouette), `Mira` / `Mira.001` (Mira herself, alive), `Tea.001` (visitor's tea), `Saucer` / `Saucer.002`, and `handbag` / `Handbag` / `Handbag.001` (Briq tote). `requiredClues` gates on `clue-l2-cup-residue` + `clue-l2-laptop` (powder + unsent Veer email → poisoning). The MCQ matches the story bible's "WEAPON: how was she killed?" question, **not** the old stub's "who was she preparing to confront?". Sibling fan-out (Tea/Saucer.002 → Saucer, frames → portraits, etc.) is not authored yet — the picker resolves only the canonical mesh names listed in `interactives`. Fan them in if testers click siblings and get nothing.
- **Three meshes the first L2 author pass relied on are no longer in level-02.glb** (May-2026 modeler rebake): `Cup` (Mira's blue mug), `Cup.001` (visitor's white mug), and `Donut`. The cup-residue clue is now routed to `Saucer` and the visitor's-cup clue to `Tea.001`; the donut clue was removed entirely. If a future rebake restores any of these meshes, the natural next moves are: route `clue-l2-cup-residue` back to `Cup`, route `clue-l2-visitor-cup` back to `Cup.001`, and reintroduce a `Donut` clue (and consider swapping `requiredClues`'s laptop entry back to `clue-l2-donut`, which is the story-bible's stronger "visitor was here" beat).
