...
| **13** | **Level 1 polish (hover/SFX, clue images)** ⏸️ deferred | Polish hover effect, add SFX on clue discovery and note save, image zoom in clue panels, transition pacing. The structural clue panel grows real visuals here. | Level 1 feels good to play through end-to-end. |
| **14** ◐ | **Author Levels 2-6 GLBs + content** (in progress: L2 done) | The bulk of the remaining work. **Level 2 done** (May 2026): `level-02.glb` ships with the L2-specific objects baked in (Donut, Culprit silhouette, Mira herself, Cup.001/Saucer.001/Tea.001 for the visitor's white guest cup, handbag/Handbag/HandbagBrand for the Briq tote), and `level-02.js` wires 15 interactives + a poisoning-themed MCQ gated on `clue-l2-cup-residue` + `clue-l2-donut`. **Level 3-6 still pending.** Each remaining level is a Blender export + a `level-NN.js` content file per the story bible. State (capo, cups, sticky notes) is baked into each `.blend`. TOD per the story's per-scene atmosphere (overcast, warm-night, sharp-day, dusk, rainy, golden-night). Test each end-to-end through clue → notes → MCQ. | All 6 levels (1-6) playable end-to-end. Each level's `requiredClues`-gated MCQ answer from the story bible works. |
...
- `js/content/levels/level-03.js` … `level-07.js` and matching authored GLBs (Step 14 — L2 already real-authored; L3-6 placeholder GLBs were deleted during the L2 cycle, so each remaining level needs both the GLB and the content file)
...
- SFX on clue discovery / note save (Step 13 polish — currently deferred)

**Done in Steps 11–12:**
- `js/engine/audio.js` ✅ (Step 11)
- `js/ui/level-transition.js` ✅ (Step 12)
- `js/content/levels/level-02.js` ✅ (Step 12 stub replaced by real-authored Level 2 in Step 14, May 2026 — 15 interactives, poisoning MCQ, sibling fan-out still pending)
...
- `Models/1room.glb` → `assets/models/level-01.glb`. `level-02.glb` is now real-authored (L2-specific objects baked in). The Level 03/04/05 placeholder GLBs were deleted during the L2 cycle; Level 6 and Level 7 GLBs were never created. Authoring all of L3-L7 (GLB + content file each) is the remainder of Step 14.
